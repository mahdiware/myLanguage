...
#define t(_tm) { .tk = TK_##_tm, .is_tk = 1, .is_mt = 0 } // token symbol
#define mt(_tm) { .tk = TK_##_tm, .is_tk = 1, .is_mt = 1 } // multi token symbol
...
static const struct rule {
	...
	r8(Func, t(FUNC), t(NAME), t(LPAR), m(Cexp), t(RPAR), t(LBRC), m(Stmt), t(RBRC))
	r8(Func, t(FUNC), t(NAME), t(LPAR), m(Expr), t(RPAR), t(LBRC), m(Stmt), t(RBRC))
	
	r4(Func, t(NAME), t(LPAR), t(RPAR), t(SCOL))
	r5(Func, t(NAME), t(LPAR), m(Cexp), t(RPAR), t(SCOL))
	r5(Func, t(NAME), t(LPAR), m(Expr), t(RPAR), t(SCOL))
	r6(Func, t(NAME), t(LPAR), m(Cexp), n(Expr), t(RPAR), t(SCOL))
	r3(Func, t(NAME), t(STRL), t(SCOL))
...
static inline bool should_shift_pre(const struct rule *const rule, const struct token *const tokens, size_t *const token_idx)
{
    ...
    if (rule->lhs == NT_Bexp && ahead->tk >= TK_EQUL && ahead->tk <= TK_MODU) {
        ...
        */
        ...
        //printf("~%d~", tokens[*token_idx+1].tk);
        if(ahead->tk == TK_STRL && tokens[*token_idx+1].tk == TK_SCOL){
        	return true;
        }
        /*if((ahead->tk == TK_LBRA || ahead->tk == TK_COMA ) && (aback->tk == TK_RBRA || aback->tk == TK_COMA) && &tokens[*token_idx+1] == TK_COMA){
        	return true;
        }*/
    ...
    } else if(rule->lhs == NT_Pexp && rule->rhs[RULE_RHS_LAST].nt == 10){
    	//printf("%d:%d", rule->rhs[RULE_RHS_LAST].nt, rule->rhs[RULE_RHS_LAST].tk);
    	if(tokens[*token_idx-4].tk == TK_NAME && ahead->tk == TK_SCOL){
    		return true;
    	}
    	if(tokens[*token_idx-4].tk == TK_NAME && ahead->tk == TK_LBRC){
    		return true;
    	}
    } else if (rule->lhs == NT_Atom && rule->rhs[RULE_RHS_LAST].tk == TK_STRL) {
    	if(ahead->tk == TK_SCOL && (aback->tk == TK_NAME /* || aback->tk == TK_PRNT*/)){
    		return true;
    	}
    	//printf("~%d~", aback->tk);
    }
...
static inline bool should_shift_post(const struct rule *const rule, const struct token *const tokens, size_t *const token_idx)
{
    ...
    if (rule->lhs == NT_Cond || rule->lhs == NT_Elif) {
        /* swallow the next "elif" or "else" in order to parser the whole chain */
...
static int reduce(const struct rule *const rule, const size_t at, const size_t size)
{
    ...
    if (reduce_at->children == old_children) {
        return free(child_nodes), PARSER_NOMEM;
    }
...
struct node parser(const struct token *const tokens, const size_t ntokens)
{
    static const struct token
        reject = { .tk = PARSER_REJECT },
        nomem  = { .tk = PARSER_NOMEM  };
